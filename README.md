...
This repository includes `nixpacks.toml`. It adds a headless JDK, installs a pinned and
checksum-verified pdf2htmlEX AppImage inside the application image, builds both runtimes, and
starts the SvelteKit and Java processes together. Configure the Coolify application as follows:
...
On Linux production builds, PDF-to-HTML conversion uses the bundled local
`.pdf2htmlex/squashfs-root/AppRun` binary. OCR likewise uses the locally installed
OCRmyPDF, Tesseract, and Poppler tools. Document-processing routes do not proxy uploaded
files to remote conversion or OCR services.
...
This repository is source-available under the PolyForm Noncommercial License 1.0.0. Personal and other non-commercial use, modification, forks, and redistribution are permitted under that license. Commercial use requires separate permission from the copyright holder. Product concepts and implementation details are evolving quickly.
