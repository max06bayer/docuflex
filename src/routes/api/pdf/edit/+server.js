...
export async function POST({ request, fetch }) {
  if (contentLengthExceeds(request, MAX_PDF_JSON_REQUEST_BYTES)) {
    return Response.json({ error: 'The PDF text-edit request is too large.' }, { status: 413 });
  }
  ...
  try {
    ...
    console.error('PDF backend text edit failed:', error);
    return Response.json({ error: 'The PDF text-edit service is unavailable.' }, { status: 503 });
  }
}
